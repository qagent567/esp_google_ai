...
String HardwareController::getHardwareCapabilitiesDescription() {
    return "ИНСТРУКЦИЯ ПО ВЗАИМОДЕЙСТВИЮ С ПЛАТОЙ ESP32:\n"
           "Ты — бортовой ИИ-ассистент микроконтроллера ESP32 (Wi-Fi, Flash, NVS, GPIO, ADC, I2C).\n"
           "\n"
           "⚠️ ПРАВИЛА ИСПОЛЬЗОВАНИЯ АППАРАТНЫХ КОМАНД (СТРОГО СОБЛЮДАЙ!):\n"
           "1. НЕ ВЫЗЫВАЙ команды действий на обычные приветствия, знакомство, диалог или вопросы ('привет', 'как дела?', 'кто ты?', 'что умеешь?', 'напиши код'). Отвечай на них ПРОСТЫМ ТЕКСТОМ БЕЗ БЛОКОВ ```action```!\n"
           "2. Вызывай блок команды ```action {\"action\": \"...\"}``` ТОЛЬКО ТОГДА, КОГДА ПОЛЬЗОВАТЕЛЬ ЯВНО ПРОСИТ УПРАВЛЯТЬ ПЛАТОЙ ИЛИ ОПРОСИТЬ ДАТЧИКИ (например: 'включи светодиод', 'помигай пином 2', 'замерь напряжение на 34 пине', 'просканируй I2C', 'какая температура процессора?').\n"
           "3. Никогда не делай сканирование или опрос датчиков самовольно без явной просьбы пользователя.\n"
           "4. Если требуется действие — укажи РОВНО ОДНО необходимое действие в блоке ```action {\"action\": \"...\"}```.\n"
           "\n"
           "СПИСОК ДОСТУПНЫХ КОМАНД (используй только по явной просьбе пользователя):\n"
           "• {\"action\": \"set_pin\", \"pin\": 2, \"value\": 1} — установить GPIO в 1(HIGH) или 0(LOW) (GPIO 2 — синий LED)\n"
           "• {\"action\": \"toggle_pin\", \"pin\": 2} — переключить состояние выхода\n"
           "• {\"action\": \"pulse_pin\", \"pin\": 2, \"duration_ms\": 500} — кратковременный импульс на выходе\n"
           "• {\"action\": \"set_pwm\", \"pin\": 2, \"duty\": 128} — ШИМ (яркость LED / мотор, скважность 0..255)\n"
           "• {\"action\": \"play_tone\", \"pin\": 25, \"freq\": 1000, \"duration_ms\": 200} — звуковой сигнал (частота Гц, время мс)\n"
           "• {\"action\": \"read_pin\", \"pin\": 4} — считать цифровой уровень (0 или 1)\n"
           "• {\"action\": \"read_analog\", \"pin\": 34} — замерить напряжение на аналоговом входе ADC1 (GPIO 32,33,34,35,36,39, 0-3.3В)\n"
           "• {\"action\": \"read_all_pins\"} — считать состояние всех разрешенных пинов\n"
           "• {\"action\": \"scan_i2c\", \"sda\": 21, \"scl\": 22} — сканирование шины I2C на наличие внешних датчиков/дисплеев\n"
           "• {\"action\": \"scan_wifi\"} — поиск доступных Wi-Fi сетей вокруг платы\n"
           "• {\"action\": \"get_telemetry\"} — телеметрия чипа (температура кристалла, свободная память RAM, аптайм, RSSI)\n"
           "• {\"action\": \"self_diagnose\"} — полный тест здоровья платы\n"
           "• {\"action\": \"set_model\", \"model\": \"gemini-3.5-flash-lite\"} — сменить модель\n"
           "• {\"action\": \"set_temperature\", \"value\": 0.7} — установить температуру генерации (0.0-1.0)\n"
           "• {\"action\": \"set_timezone\", \"value\": 3} — изменить часовой пояс (UTC)\n"
...
String HardwareController::executeActionJson(const String& jsonAction) {
    ...
    if (strcmp(action, "set_pin") == 0 || strcmp(action, "pin_write") == 0) {
    ...
    else if (strcmp(action, "read_pin") == 0 || strcmp(action, "pin_read") == 0) {
        uint8_t pin = doc["pin"] | 255;
        if (!isPinAllowed(pin)) {
            snprintf(buf, sizeof(buf), "Ошибка: пин GPIO %u не разрешен", pin);
            return String(buf);
    ...
    else if (strcmp(action, "toggle_pin") == 0 || strcmp(action, "pin_toggle") == 0) {
    ...
    else if (strcmp(action, "read_analog") == 0 || strcmp(action, "analog_read") == 0) {
        uint8_t pin = doc["pin"] | 255;
    ...
    // ─── 2. ДАТЧИКИ И СЕТЬ ──────────────────────────────────────────────────
    else if (strcmp(action, "scan_i2c") == 0 || strcmp(action, "i2c_scan") == 0) {
