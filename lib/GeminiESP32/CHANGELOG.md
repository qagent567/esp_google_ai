...
#### Исправлено и Оптимизировано (Предрелизный аудит)
- **Устранение дублирования кода (DRY):** Полностью удалены избыточные копии классов из корневых директорий (`src/` и `include/`). Прошивка-пример теперь прозрачно использует `lib/GeminiESP32`.
- **Исправление Wi-Fi:** В `NetworkManager` исправлен баг аутентификации (теперь корректно выставляется `WIFI_AUTH_WPA_PSK` при наличии пароля сети).
- **Оптимизация памяти:** Добавлен предварительный аллокатор `jsonOutput.reserve(2048)` при сериализации JSON-запросов для предотвращения фрагментации кучи.
